...
Let $R$ be a commutative ring with unity (CRWU). Let $V,W$ be finite-dimensional $R$–modules with
\[
...
\begin{block}{Kronecker Delta}
    Let $V$ be finite-dimensional with ordered basis $e=(e_1,\dots,e_n)$. The \emph{dual basis} $e^*=(e^1,\dots,e^n)$ is the family of linear forms $e^i:V\to R$ defined by
\[
...
Let $V$ be finite-dimensional, $e=(e_1,\dots,e_n)$ a basis and $e^*=(e^1,\dots,e^n)$ its dual. Then for every $v\in V$,
\[
...
Let $R$ be a CRWU, $V,W$ finite-dimensional $R$–modules with ordered bases
\[
...
\begin{block}{Matrix of $\phi^*$ is the Transpose of $(\phi)_e^f$}
    Let $V,W$ be finite-dimensional, $e=(e_1,\dots,e_n)$, $f=(f_1,\dots,f_m)$ with dual bases $e^*=(e^1,\dots,e^n)$, $f^*=(f^1,\dots,f^m)$. For $\phi\in\operatorname{Hom}_R(V,W)$,
\[
...
\begin{block}{Finite-dimensional Case} If $V$ is finite-dimensional, then $\iota$ is an isomorphism; we write $V \simeq V^{**}$ \emph{canonically}.
...
\vspace{-0.5cm}
\begin{block}{}
Let $V,W$ be finite-dimensional, and $\phi\in\operatorname{Hom}_R(V,W)$. The double dual map $\phi^{**}:V^{**}\to W^{**}$ is defined by
\[
